...
from util.logging import args2 as args
from util.meta import side_list as label_list
import torch
from torch.utils.data import TensorDataset, DataLoader, RandomSampler  # , SequentialSampler
from util.data_processing import convert_examples_to_features
from util.data_structure import PartisanTextProcessor as LabelTextProcessor
...
def fit(model, train_dataloader, test_dataloader, optimizer, num_train_steps, num_epocs=5):
    ...
    writer_name = os.path.join(ROOT, 'log', 'part' + time_string)
    print(writer_name)
    writer = SummaryWriter(writer_name)
    n_gpu = 1
    global_step = 0
    output_model_file = os.path.join(ROOT, 'save', "part_pytorch_model_3.bin")
    ...
    for i_ in trange(int(num_epocs), desc="Epoch"):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            if (step + 1) % 10 == 0:
                ...
                writer.add_scalar('train/loss', torch.mean(loss).detach().cpu().numpy(), count)
            if (step + 1) % 100 == 0:
                r = set_eval(model, test_dataloader)
                writer.add_scalar('eval/loss', r['eval_loss'], count)
                writer.add_scalar('eval/acc', r['eval_accuracy'], count)
